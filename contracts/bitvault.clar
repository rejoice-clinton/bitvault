    ...
    vault (match (get-price "STX")
      stx-price (match (get-price "xBTC")
        xbtc-price (let (
          ...
          (if (is-eq debt u0)
            (ok u999999) ;; Infinite health factor with no debt
...
(define-public (liquidate-vault (vault-id uint))
  (let (
    ...
    ;; Calculate collateral distribution with liquidation penalty
    (let (
...
;; PROTOCOL QUERY AND ANALYTICS FUNCTIONS
